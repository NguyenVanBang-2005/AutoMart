...
async function handleLoginSubmit(e) {
  ...
  try {
    ...
    if (!res.ok) { showToast(data.detail || 'Đăng nhập thất bại!'); return; }
    showToast(`Chào mừng, ${data.user.ho_ten}!`);
    closeModal('loginModal'); form.reset();
...
async function handleRegisterSubmit(e) {
  ...
  if (data.password !== data.confirmPassword) { showFormError('registerError', 'Mật khẩu xác nhận không khớp!'); return; }
  if (data.password.length < 6)               { showFormError('registerError', 'Mật khẩu phải có ít nhất 6 ký tự!'); return; }
  if (!data.agree)                             { showFormError('registerError', 'Vui lòng đồng ý với điều khoản!'); return; }
  ...
  try {
    const res = await fetch(`${API_BASE}/users/register`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      credentials: 'include',
      body: JSON.stringify({ ho_ten: data.fullName, email: data.email, so_dien_thoai: data.phone, password: data.password })
    });
    const result = await res.json();
    if (!res.ok) { showFormError('registerError', result.detail || 'Đăng ký thất bại!'); return; }
    showToast(`Chào mừng ${result.user.ho_ten}!`);
    closeModal('registerModal'); form.reset(); hideFormError('registerError');
    setTimeout(() => window.location.reload(), 800);
  } catch { showFormError('registerError', 'Không thể kết nối server!'); }
  finally { btn.classList.remove('btn-loading'); btn.disabled = false; }
}
...
function tryOpenSellModal() {
  ...
  if (isLoggedIn) { openModal('sellModal'); }
  else { showToast('Vui lòng đăng nhập để đăng tin bán xe'); openModal('loginModal'); }
}
...
async function handleSellSubmit(e) {
  ...
  const payload = {
    hang_xe: selects[0]?.value || '', dong_xe: inputs[0]?.value || '',
    nam_san_xuat: parseInt(selects[1]?.value) || 0, so_km: parseInt(inputs[1]?.value) || 0,
    gia_mong_muon: parseInt(inputs[2]?.value) || 0, khu_vuc: selects[2]?.value || '',
    mo_ta: form.querySelector('textarea')?.value || '',
    ho_ten: inputs[3]?.value || '', so_dien_thoai: inputs[4]?.value || ''
  };
  try {
    ...
    const data = await res.json();
    if (!res.ok) { showToast(data.detail || 'Đăng tin thất bại!'); return; }
    showToast('Đăng tin thành công!'); closeModal('sellModal'); form.reset();
  } catch { showToast('Không thể kết nối server!'); }
}
...
function hideFormError(elementId) {
  const el = document.getElementById(elementId); if (el) el.style.display = 'none';
}

// ── Scroll Row ───────────────────────────────────────
function scrollRow(rowId, dir) {
  const row = document.getElementById(rowId);
  if (!row) return; row.scrollBy({ left: dir * 260, behavior: 'smooth' });
}
...
function handleNewsletterSubmit(e) {
  ...
  if (!input?.value) return; showToast('Đăng ký nhận tin thành công!'); input.value = '';
}
...
// ── viewCar → redirect sang trang chi tiết ───────────
function viewCar(id) {
...
async function loadCarsFromAPI(filters = {}) {
  try {
    ...
    allCars = data.cars.map(car => ({
      id: car.id, brand: car.hang || '', model: car.dong || '',
      year: car.nam || 0, price: car.gia || 0, km: car.km || 0,
      fuel: car.nhien_lieu || 'Xăng', trans: car.hop_so || 'Tự động',
      location: car.khu_vuc || '', badge: car.badge || '',
      color: car.color || '#6366f1', image_url: car.anh || ''
    }));
    return allCars;
  } catch (err) {
    console.warn('Dùng dữ liệu mẫu:', err);
    allCars = carsData; return allCars;
  }
}
...
function filterBrand(brand) {
  ...
  if (filtered.length > 0) {
    renderCarsData(filtered); showToast(`Hiển thị ${filtered.length} xe ${brand}`);
  } else {
    loadCarsFromAPI({ hang: brand }).then(cars => {
      renderCarsData(cars); showToast(`Hiển thị ${cars.length} xe ${brand}`);
    });
  }
...
async function handleTuVanSubmit(e) {
  ...
  const btn  = form.querySelector('button[type="submit"]');
  const data = Object.fromEntries(new FormData(form));
  if (!data.ho_ten || !data.so_dien_thoai) { showToast('Vui lòng điền họ tên và số điện thoại!'); return; }
  if (btn) { btn.disabled = true; btn.classList.add('btn-loading'); }
  try {
    const res    = await fetch(`${API_BASE}/tu-van`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, credentials: 'include', body: JSON.stringify(data) });
    const result = await res.json();
    if (!res.ok) { showToast(result.detail || 'Gửi thất bại!'); return; }
    showToast('Đăng ký tư vấn thành công!'); form.reset();
  } catch { showToast('Không thể kết nối server!'); }
...
async function handleLienHeSubmit(e) {
  ...
  const btn  = form.querySelector('button[type="submit"]');
  const data = Object.fromEntries(new FormData(form));
  if (!data.ho_ten || !data.email || !data.noi_dung) { showToast('Vui lòng điền đầy đủ thông tin!'); return; }
  if (btn) { btn.disabled = true; btn.classList.add('btn-loading'); }
  try {
    const res    = await fetch(`${API_BASE}/lien-he`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, credentials: 'include', body: JSON.stringify(data) });
    const result = await res.json();
    if (!res.ok) { showToast(result.detail || 'Gửi thất bại!'); return; }
    showToast('Gửi liên hệ thành công!'); form.reset();
  } catch { showToast('Không thể kết nối server!'); }
  finally { if (btn) { btn.disabled = false; btn.classList.remove('btn-loading'); } }
}
...
function switchLoginPanel(panel) {
  ...
  social.style.display = panel === 'social' ? 'block' : 'none';
  email.style.display  = panel === 'social' ? 'none'  : 'block';
}
